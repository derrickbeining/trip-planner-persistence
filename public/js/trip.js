...
var tripModule = (function () {
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~
  function createDayAndDisplay (dayInstance) {
    var newDay = dayModule.create(dayInstance); // dayModule
    days.push(newDay);
    if (days.length === 1) {
      currentDay = newDay;
    }
    ...
    if (dayData) {
      ...
      $.post('/api/days', { number: days.length + 1 })
        ...
        .catch(reason => {
          ...
          alert('Day was not successfully created, try again.')
        })
    }
  }
